...
class UpdateActionUsecase:
    ...
    def __call__(self, action_id: str,
        ...
        needs_member_update = members != None and set(members) != set([action.user_id] + action.associated_members_user_ids)
        needs_date_update = start_date != action.start_date
        
        if needs_date_update:
            self.repo.batch_update_associated_action_start(action_id, new_start_date=start_date)
